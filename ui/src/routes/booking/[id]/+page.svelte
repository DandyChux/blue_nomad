<script lang="ts">
	...
	import type { SearchAvailabilityResponse } from "$lib/schemas";
	...
	const price = $derived(
		(
			(variation?.item_variation_data?.price_money?.amount || 0) / 100
		).toFixed(0),
	);
	...
	<section class="min-h-screen pt-28 lg:pt-36 px-6 md:px-12 lg:px-16 pb-20">
		...
		>
			...
			<div class="flex flex-col gap-4 lg:sticky lg:top-36 lg:self-start">
				<div
					class="relative w-full aspect-[3/4] bg-muted overflow-hidden"
				>
						...
						>
							<Picture
								src={imageUrls[activeImageIndex] ||
									service.image_url}
								alt="{itemData.name} - Image {activeImageIndex +
									1}"
			...
			<div class="flex flex-col gap-8 lg:py-8">
				...
				<!-- Step Indicator -->
				<div
					class="font-source-code-pro text-[10px] uppercase tracking-widest text-muted-foreground"
				>
				...
				{#if booking.step === 1}
					<div class="space-y-6" in:fade>
						<h2
							class="uppercase text-2xl tracking-tighter font-light"
						>
							Select a Date
						</h2>
						...
						{#if booking.date}
							<div class="space-y-4" in:fade>
								...
								{:else if booking.slots.length === 0}
									<p
										class="text-center text-xs font-source-code-pro uppercase tracking-widest text-muted-foreground py-8"
									>
				...
				{:else if booking.step === 3}
					<div class="space-y-8" in:fade>
						...
						{#if booking.error}
							<p
								class="text-red-500 text-sm font-source-code-pro"
							>
								{booking.error}
							</p>
						{/if}
